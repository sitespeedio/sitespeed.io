...
There are a couple of ways to create queries and alerts:
...
You see that we run the alerts once an hour. It depends on how often you do releases or content changes. You want to make sure that you catch the alerts within at least a couple of hours.


### Create metrics queries
The other way is to create alerts that fire if a threshold is met. In this example we want to alert if the First Visual Change increases by 20 ms for three URLs. The graph looks like this:

![Alert graph]({{site.baseurl}}/img/alerts/alerts-difference-graph.png){:loading="lazy"}
{: .img-thumbnail-center}

And setting up the graph is more straightforward than using percentages. You get the metric you want, diff the metric with X amount back in time and draw the difference.

![Alert query setup]({{site.baseurl}}/img/alerts/alerts-difference.jpg){:loading="lazy"}
{: .img-thumbnail}

#### The alert
Then you set up the alert. In this example we run the alert query once every hour and it needs to fire twice within 2 hours to actually send an alert. If we make sure we run our tests at least every hour, we need two runs with higher values than the limit to fire the alert.
...
The history graph is pretty straightforward. You list the metrics you want and you configure how far back in time you want to graph them. We used to do 30 days (which is really good for seeing trends), but it was too long to see something when an actual regression was happening, now we use 7 days.
...
You can also create alerts that fire when a response type's size increases. Here we graph the JavaScript and CSS size.
...
We know it shouldn't happen, but sometimes your page references a 404 or a 50x. Let's alert on that!
...
Set up your query something like this:
...
#### Alert on too low privacy

One of the most important metrics you can get from the Coach is the privacy metric that helps you see how well you take care of your users and whether you share their private information with other companies/websites.
...
And then when you set up the alert, make sure you alert on values *below* your current value.
...
You can do the same with all the metrics you want. On mobile, Wikipedia metrics are more stable and the First Visual Change looks like this:

![First visual change]({{site.baseurl}}/img/alerts/first-visual-change2.png){:loading="lazy"}
{: .img-thumbnail}

If you have any questions about the alerts, feel free to [create an issue on GitHub](https://github.com/sitespeedio/sitespeed.io/issues/new?title=Alerts) or hit us on [Slack](https://join.slack.com/t/sitespeedio/shared_invite/zt-296jzr7qs-d6DId2KpEnMPJSQ8_R~WFw).
