...
Each row shows:
1. **Metric name and p-value**: the metric that was tested and the score from the Mann-Whitney U/Wilcoxon test. A score below 0.05 means the difference is statistically significant. When several metrics in a group carry exactly the same run values (the before-FCP and before-LCP pairs do whenever FCP and LCP are the same paint), they merge into one row tagged *identical values* so the same finding is not shown and counted twice.
