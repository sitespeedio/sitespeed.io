...
Browser
  ...
  -r, --browsertime.requestheader, --requestheader                                                  Request header that will be added to the request. Add multiple instances to add multiple request headers. Use the following format key:value. Only works in Chrome, Firefox and Edge.
      ...
      --browsertime.videoParams.noiseTolerance, --videoParams.noiseTolerance                        Percentage (0-100) of pixels allowed to differ between video frames before Visual Metrics still treats a frame as unchanged. Browsers can re-rasterize a visually done page long after it settles, leaving invisible sub-pixel noise on glyph edges that inflates Last Visual Change, Speed Index and VisualComplete95/99. Try 0.05 if you see a too late Last Visual Change with no visible change in the filmstrip. 0 (default) keeps the old behavior of allowing 5 pixels.  [number] [default: 0]
...
Filmstrip
      ...
      --browsertime.videoParams.filmstripDiff, --videoParams.filmstripDiff          Write a diff image for each filmstrip frame (the frame dimmed with every pixel that changed since the previous frame painted red) plus a Visual instability heatmap of the whole recording. The HTML report shows both on the Rendering tab, so you can see exactly where and how often the page repainted. Adds post-processing time and one extra image per frame.  [boolean] [default: false]
...
Chrome
      ...
      --browsertime.chrome.coverage, --chrome.coverage                                Collect JavaScript and CSS code coverage on every iteration. Per-iteration summaries with a per-file breakdown land in the per-URL result and the HTML report surfaces them under the PageXray tab. This has a real performance cost: detailed JS coverage causes V8 to deoptimize and will skew timing metrics like LCP, TBT and INP. Use --enableProfileRun instead if you want coverage without affecting your timing data; it collects coverage during the extra profile iteration only.  [boolean] [default: false]
...
compare
      ...
      --compare.mannwhitneyu.method         Selects the method for calculating the Mann-Whitney U test. auto automatically selects between exact and asymptotic based on sample size, exact uses the exact distribution of U, and asymptotic uses a normal approximation.  [choices: "auto", "exact", "asymptotic"] [default: "auto"]
...
API
      ...
      --api.port      The port for the API  [default: 3000]
