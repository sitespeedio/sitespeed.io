  ...
  if (pageData.phantomjs) {
    ...
    Object.keys(pageData.phantomjs.timings).forEach(function(metric) {
      if (timeMetrics.hasOwnProperty(metric+'PhantomJS')) {
        timeMetrics[metric+'PhantomJS'].push(Number(pageData.phantomjs.timings[metric]));
      } else {
        timeMetrics[metric+'PhantomJS'] = new Stats().push(Number(pageData.phantomjs.timings[metric]));
      }
    });
