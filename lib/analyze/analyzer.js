...
Analyzer.prototype.analyze = function(urls, collector, config, downloadErrors, analysisErrors, urlAnalysedCallback,
  ...
  */
  ...
  var tasks = [
    ...
    function(cb) {
      if (config.phantomjs) {
        phantomjs.analyze(urls, config, cb);
      } else {
        cb(undefined, {});
      }
    },
    ...
    function(cb) {
      if (config.browsertime) {
        browsertime.analyze(urls, config, cb);
      } else {
        cb(undefined, {});
      }
    },
    function(cb) {
      if (config.wptUrl) {
        webpagetest.analyze(urls, config, cb);
      } else {
        cb(undefined, {});
      }
    },
    function(cb) {
      if (config.screenshot) {
        screenshots.takeScreenshots(urls, config, cb);
      } else {
        cb(undefined, {});
      }
    }
