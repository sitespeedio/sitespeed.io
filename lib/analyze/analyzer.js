  ...
  */
  ...
  var extractDataForType = function(type, data) {
    ...
    switch (type) {
      case 'browsertime': {
        result = {};
        data.forEach(function(runPerBrowser) {
          var btData = result.browsertime || [];
          btData.push(runPerBrowser.browsertime);
          result.browsertime = btData;

          var har = result.har || [];
          har.push(runPerBrowser.har);
          result.har = har;
        });
      }
        break;
