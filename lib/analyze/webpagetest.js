  ...
  wpt.runTest(url, wptOptions, function(err, data) {
    ...
    async.parallel({
      fetchWaterfall: function(cb) {
        ...
        wpt.getWaterfallImage(id, waterfallOptions, function(wtfIErr, data, info) {
          if (wtfIErr) {
            log.log('error', 'WebPageTest couldn\'t get waterfall image ' + wtfIErr);
          }
        ...
        fetchRepeatWaterfall: function(cb) {
          ...
          wpt.getWaterfallImage(id, waterfallOptions, function(err2, data, info) {
          ...
          fetchHar: function(cb) {
            wpt.getHARData(id, {}, function(err3, data) {
              if (err3) {
                ...
                var harPath = path.join(config.run.absResultDir, config.dataDir,
                  ...
                  fs.writeFile(harPath, JSON.stringify(data), function(err4) {
                    if (err4) {
