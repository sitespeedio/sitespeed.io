...
AnalyzeMultipleSites.prototype.run = function(finishedCb) {
  ...
  this.log.log('info', 'Analyze ' + urls.length + ' sites');
  urls.forEach(function(url) {
    if (url !== '') {
      queue.push({
        ...
        self.log.log('info', 'Finished with site ' + url);
      });
    }
  });
    ...
    queue.drain = function() {
      var now = moment();
      self.log.log('info', 'The full site analyze took ' + now.from(self.startTime, true) + ' (' + now.diff(self.startTime, 'seconds') + ' seconds)');
