...
AnalyzeOneSite.prototype.run = function(callback) {
  ...
  **/
  async.waterfall([
    ...
    var start = moment(self.config.run.date);
    var now = moment();
    self.log.log('info', 'The analyze took ' + now.diff(start,'seconds') + ' seconds (' + now.from(moment(start), true) + ')');
...
AnalyzeOneSite.prototype._analyze = function(urls, downloadErrors, callback) {
  var analysisErrors = {};
  var self = this;
  this.log.log('info', 'Will analyze ' + urls.length + ' pages');
...
AnalyzeOneSite.prototype._createOutput = function(downloadErrors, analysisErrors, callBack) {
