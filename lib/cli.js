...
var fs = require('fs-extra'),
  ...
  cli = require('nomnom').help(
    ...
    threshold: {
      metavar: '<FILE>',
      help: 'A file containing JSON like  {\"rules\": {\"default\": 88}}',
      callback: function(file) {
        if (!fs.existsSync(file)) {
          return 'Couldnt find the threshold JSON file:' + fs.realpathSync(file);
        }
      }
    },
    maxPagesToTest: {
    ...
    requestHeaders: {
      metavar: '<FILE>',
      help: 'Any request headers to use, in the JSON form of {\"name\":\"value\",\"name2\":\"value\"}. Not supported for WPT & GPSI.',
      callback: function(file) {
        if (!fs.existsSync(file)) {
...
if (cli.threshold) {
  cli.threshold = fileHelper.getFileAsJSON(cli.threshold);
}
