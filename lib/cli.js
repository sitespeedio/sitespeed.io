 ...
 */
'use strict';
/*eslint eqeqeq:0, no-process-exit:0, no-mixed-requires: 0*/
...
var fs = require('fs-extra'),
  ...
  cli = require('nomnom').help(
      ...
      callback: function(file) {
        if (!fs.existsSync(file)) {
          fs.mkdirs(file, function(err) {
            if (err) {
              return 'Couldn\'t create the result base dir:' + err;
            }
          });
        }
      }
    ...
    testData: {
      ...
      help: 'Choose which data to send test when generating TAP/JUnit output. Default is all available [rules,timings,wpt,gpsi]'
    },
    budget: {
      metavar: '<FILE>',
    ...
    postTasksDir: {
      ...
      callback: function(file) {
        if (!fs.existsSync(file)) {
          return 'Couldn\'t find the dir:' + fs.realpathSync(file);
        }
      }
    },
    boxes: {
