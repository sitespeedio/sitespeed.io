...
module.exports.parseCommandLine = function parseCommandLine() {
  let parsed = yargs
    ...
    .option('browsertime.visualMetrics', {
      ...
      type: 'boolean',
      describe: 'Calculate Visual Metrics like SpeedIndex, First Visual Change and Last Visuaö Change. Requires FFMpeg and Python dependencies',
      group: 'Browser'
    })
    .option('browsertime.video', {
      alias: 'video',
      type: 'boolean',
      describe: 'Record a video and store the video. Set it to false to remove the video that is created by turning on visualMetrics. To remove fully turn off video recordings, make sure to set video and visualMetrics to false. Requires FFMpeg to be installed.',
      group: 'Browser'
    })
