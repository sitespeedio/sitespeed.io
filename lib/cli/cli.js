...
import { toArray, setProperty } from '../support/util.js';
...
export async function parseCommandLine() {
  ...
  explicitOptions = Object.entries(explicitOptions).reduce(
    (result, [key, value]) => {
      if (aliasLookup.has(key)) {
        ...
        result = setProperty(result, fullKey, value);
      }
  ...
  if (argv.spa) {
    ...
    setProperty(argv, 'browsertime.useHash', true);
  }
  ...
  if (
    ...
    setProperty(argv, 'browsertime.chrome.collectLongTasks', true);
    setProperty(argv, 'browsertime.chrome.timeline', true);
  }
  ...
  if (argv.webpagereplay) {
    setProperty(argv, 'browsertime.webpagereplay', true);
  }
  ...
  if (argv.browsertime.docker) {
    setProperty(
      argv,
      'browsertime.video',
      get(argv, 'browsertime.video', true)
    );
    setProperty(
      argv,
  ...
  if (argv.browsertime.safari && argv.browsertime.safari.useSimulator) {
    setProperty(argv, 'browsertime.connectivity.engine', 'throttle');
  } else if (
  ...
  if (argv.urlAlias) {
    ...
    setProperty(argv, 'browsertime.urlMetaData', meta);
  ...
  if (argv.debug) {
    setProperty(argv, 'browsertime.timeouts.pageload', 2_147_483_647);
    setProperty(argv, 'browsertime.timeouts.script', 2_147_483_647);
    setProperty(argv, 'browsertime.timeouts.pageCompleteCheck', 2_147_483_647);
  }
