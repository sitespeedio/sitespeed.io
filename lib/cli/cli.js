...
const pageMetrics = require('../support/friendlynames');
const metricList = Object.keys(pageMetrics);

const DEFAULT_PAGE_METRIC_LENGTH = 7;
...
function validateInput(argv) {
  ...
  if (!Array.isArray(argv.html.pageSummaryMetrics)) {
    return (
      'Something looks wrong with your page summary metrics. ' +
      'Expected array of metrics to test URL(s).'
    );
  }

  if (
    Array.isArray(argv.html.pageSummaryMetrics) &&
    argv.html.pageSummaryMetrics.length > DEFAULT_PAGE_METRIC_LENGTH
  ) {
    return 'Displayed Page metrics limited to 7 max';
  }

  if (Array.isArray(argv.html.pageSummaryMetrics)) {
    for (let metric of argv.html.pageSummaryMetrics) {
      const [m, k] = metric.split('.');
      if (
        !metricList.some(
          tools => pageMetrics[tools][m] && pageMetrics[tools][m][k]
        )
      ) {
        return `Require test metrics to be from ${path.resolve(
          './lib/support/friendlynames'
        )}`;
      }
    }
  }
   ...
   */
    ...
    */
    ...
    .option('html.pageSummaryMetrics', {
      type: 'array',
      describe:
        'Select from a list of test metrics to be displayed for given URL(s).' +
        ' Here is a sample format --html.pageSummaryMetrics request.total score.performance',
      group: 'HTML',
