export function addOptions(yargs) {
  yargs
    ...
    .option('browsertime.chrome.coverage', {
      alias: 'chrome.coverage',
      type: 'boolean',
      default: false,
      describe:
        'Collect JavaScript and CSS code coverage on every iteration. ' +
        'Per-iteration summaries with a per-file breakdown land in the per-URL ' +
        'result and the HTML report surfaces them under the PageXray tab. This has ' +
        'a real performance cost: detailed JS coverage causes V8 to deoptimize and ' +
        'will skew timing metrics like LCP, TBT and INP. Use --enableProfileRun ' +
        'instead if you want coverage without affecting your timing data — it ' +
        'collects coverage during the extra profile iteration only.',
      group: 'Chrome'
    })
