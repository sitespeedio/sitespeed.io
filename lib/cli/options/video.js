export function addOptions(yargs) {
  yargs
    ...
    .option('browsertime.videoParams.filmstripDiff', {
      alias: 'videoParams.filmstripDiff',
      type: 'boolean',
      default: false,
      describe:
        'Write a diff image for each filmstrip frame (the frame dimmed with every pixel that changed since the previous frame painted red) plus a Visual instability heatmap of the whole recording. The HTML report shows both on the Rendering tab, so you can see exactly where and how often the page repainted. Adds post-processing time and one extra image per frame.',
      group: 'Filmstrip'
    })
