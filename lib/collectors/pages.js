...
function collectYSlowMetrics(pageData, p) {
  ...
  var assetTypes = ['js', 'css', 'image', 'cssimg', 'font', 'flash', 'iframe','doc'];
...
function collectWPT(pageData, p) {
  ...
  views.forEach(function(view) {
    ...
    sizes.forEach(function(size) {
      p.wpt[view][size] = {
        'v': pageData.webpagetest.response.data.median[view][size],
        'unit': 'bytes'
      };
    });
    timings.forEach(function(timing) {
      p.wpt[view][timing] = {
        'v': pageData.webpagetest.response.data.median[view][timing],
        'unit': 'milliseconds'
      };
    });
    others.forEach(function(metric) {
      p.wpt[view][metric] = {
        'v': pageData.webpagetest.response.data.median[view][metric],
        'unit': ''
      };
    });
