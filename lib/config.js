...
exports.setupConfiguration = function(config) {
  ...
  if (config.runYslow) {
    ...
    config.sitesColumns = ['yslow.requests', 'yslow.criticalPathScore', 'yslow.pageWeight', 'yslow.cacheTime',
      'yslow.ruleScore'
    ];
  ...
  if (config.gpsiKey) {
    config.summaryBoxes.push('gpsiscore');
  ...
  if (config.wptUrl) {
    config.summaryBoxes.push('firstViewTTFB', 'firstViewSpeedIndex',
      'repeatViewSpeedIndex',
      'firstViewVisualComplete', 'repeatViewVisualComplete');
    config.sitesColumns.push('wpt.firstView.SpeedIndex', 'wpt.repeatView.SpeedIndex');
  ...
  if (config.browsertime)  {
    ...
    config.sitesColumns.push('browsertime.serverResponseTime', 'browsertime.domContentLoadedTime');
