...
exports.setupConfiguration = function(config) {
  ...
  if (!config.resultBaseDir) {
    config.resultBaseDir = defaultConfig.resultBaseDir;
  }
  ...
  if (!config.deep) {
    config.deep = defaultConfig.deep;
  }
  ...
  if (!config.viewPort) {
    config.viewPort = defaultConfig.viewPort;
  }

  if (!config.yslow) {
    config.yslow = defaultConfig.yslow;
  }

  if (!config.ruleSet) {
    config.ruleSet = defaultConfig.ruleSet;
  }

  if (!config.limitFile) {
    config.limitFile = defaultConfig.limitFile;
  }

  if (!config.profile) {
    config.profile = defaultConfig.profile;
  }

  if (!config.no) {
    config.no = defaultConfig.no;
  }

  if (!config.graphitePort) {
    config.graphitePort = defaultConfig.graphitePort;

  }
  if (!config.graphiteNamespace) {
    config.graphiteNamespace = defaultConfig.graphiteNamespace;
  }

  if (!config.graphiteData) {
    config.graphiteData = defaultConfig.graphiteData;
  }
