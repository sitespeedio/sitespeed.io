...
Graphite.prototype.sendPageData = function(aggregates, pages, config, cb) {
  ...
  server.addListener('error', function(connectionException) {
    self.log.log('error', 'Couldn\'t send data to Graphite:' + connectionException + ' for host:' + self.host + ' port:' +
      self.port);
  });
  ...
  aggregates.forEach(function(aggregate) {
    values.forEach(function(value) {
      ...
      if (aggregate.id.indexOf('WPT')>-1) {
        ...
        aggregate.id.replace('WPT','').replace('firstView','firstView.').replace('repeatView','repeatView.') +
