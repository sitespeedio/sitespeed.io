...
Graphite.prototype.sendPageData = function(aggregates, pages, config, cb) {
  ...
  this.log.verbose('Send the following keys to Graphite:', statistics);
  ...
  server.on('connect', function() {
    self.log.log('info', 'Sending data to Graphite ...');
  ...
  aggregates.forEach(function(aggregate) {
    values.forEach(function(value) {
      ...
      if (aggregate.id.indexOf('WPT') > -1) {
        // There's some hacking going on here and the reason is that the key in the aggregator don't
        // hold dots (.) since we want to be able to store it simple in Mongo
        statistics += namespace + '.summary.' + hostname + '.' + aggregate.type + '.wpt.' +
          aggregate.id.replace('WPT', '').replace('firstView', 'firstView.').replace('repeatView',
            'repeatView.') +
          '.' + value + ' ' + aggregate.stats[value] + timeStamp;
      } else {
...
Graphite.prototype._getPageStats = function(page, namespace, timeStamp) {
  ...
  if (this.config.graphiteData.indexOf('pagemetrics') > -1 || this.config.graphiteData.indexOf(
    ...
    if (page.yslow) {
      ...
      statistics += namespace + '.' + urlKey + 'pageWeight' + ' ' + page.yslow
        .pageWeight.v + timeStamp;
    }
  }
        ...
        Object.keys(page.wpt[location][browser]).forEach(function(connectivity) {
          Object.keys(page.wpt[location][browser][connectivity]).forEach(function(view) {
            Object.keys(page.wpt[location][browser][connectivity][view]).forEach(function(metric) {
                statistics += namespace + '.' + urlKey + 'wpt.' + location + '.' +
                  browser + '.' + connectivity + '.' + view + '.' + metric + '.median ' +
