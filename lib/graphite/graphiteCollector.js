...
GraphiteCollector.prototype._getDomainStats = function(domains, hostname) {
	...
	if (domains) {
		...
		domains.forEach(function(domain) {
			...
			stats += self.namespace + '.summary.' + hostname + '.domains.' + domain.domain.split('.').join('_') + '.requests' +
				' ' + domain
				.count +
				self.timeStamp;
			// and the size
			stats += self.namespace + '.summary.' + hostname + '.domains.' + domain.domain.split('.').join('_') + '.totalWeight' +
				' ' + domain
				.totalWeight +
				self.timeStamp;
		});
