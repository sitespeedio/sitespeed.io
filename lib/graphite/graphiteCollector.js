...
GraphiteCollector.prototype._getDomainStats = function(domains, hostname) {
	...
	if (domains) {
		...
		domains.forEach(function(domain) {
			...
			stats += self.namespace + '.summary.' + hostname + '.domains.totalRequests.' + domain.domain.split('.').join('_') + '' +
				' ' + domain
				.count +
				self.timeStamp;
			// and the size
			Object.keys(domain.size).forEach(function(size) {
				stats += self.namespace + '.summary.' + hostname + '.domains.totalSize.' + domain.domain.split('.').join('_') + '.' + size +
					' ' + domain.size[size] +
					self.timeStamp;
			});
		});
