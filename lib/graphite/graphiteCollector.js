...
GraphiteCollector.prototype._getDomainStats = function(domains, hostname) {
	...
	if (domains) {
		...
		domains.forEach(function(domain) {
			...
			stats += self.namespace + '.summary.' + hostname + '.domains.' + domain.domain.split('.').join('_') + '.totalrequests' +
				' ' + domain
				.count +
				self.timeStamp;
			// and the size
			Object.keys(domain.size).forEach(function(size) {
				stats += self.namespace + '.summary.' + hostname + '.domains.' + domain.domain.split('.').join('_') + '.totalsize.' + size +
					' ' + domain.size[size] +
					self.timeStamp;
			});
		});
