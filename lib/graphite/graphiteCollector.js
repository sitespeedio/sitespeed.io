...
// maybe later we want to send specific asset data
/*
GraphiteCollector.prototype._getAssetsStats = function (page, urlKey) {
  ...
  page.har.forEach(function(har) {
    har.log.entries.forEach(function(entry) {
      var assetURL =  util.getGraphiteURLKey(decodeURIComponent(entry.request.url));
      var total;
        if (entry.timings) {
        ...
        stats +=  self.namespace + '.' + urlKey + 'assets.' +  assetURL + ' ' + total + self.timeStamp;
		...
		domains.forEach(function(domain) {
			timings.forEach(function(timing) {
				values.forEach(function(value) {
					stats += self.namespace + '.summary.' + hostname + '.domains.timings.' + domain.domain + '.' + timing + '.' +
						value + ' ' + domain[timing].stats[value] + self.timeStamp;
				});
			});
			stats += self.namespace + '.summary.' + hostname + '.domains.' + domain.domain + '.requests' + ' ' + domain.count +
				self.timeStamp;
		});
	}
