...
module.exports.registerHelpers = function registerHelpers() {
  ...
  hb.registerHelper('showMaxEnd', function(string, length) {
    string = util.decodeURIComponent(string);
    if (string.length > length) {

      return string.substring(0, 25) + '...' + string.substring(string.length - length - 25, string.length);
    }
    return string;
  });
  ...
  hb.registerHelper('getColumnsMeta', function(column, columnsMeta, ruleDictionary, type) {
    ...
    } else if (column.indexOf('phantomjs.') === 0) {
      column = column.replace('phantomjs.', '');
    }
