...
HTMLRenderer.prototype.renderToplist = function(assets, pages, domains, cb) {

  // TODO cleanup
  var limit = 10;

  var bySize = assets.sort(function(asset, asset2) {
    return asset2.size - asset.size;
  });

  var largestImages = bySize.filter(function(asset) {
    return (asset.type === 'image' || asset.type === 'cssimage');
  });

  // sometimes the last mod is -1 if it is not set by the server
  var worstCachedAssets = assets.filter(function(asset) {
    return asset.timeSinceLastModification !== -1;
  });

  // sometimes the last mod is -1 if it is not set by the server

  worstCachedAssets.sort(function(asset, asset2) {
  ...
  if (worstCachedAssets.length > limit) {
    worstCachedAssets.length = limit;
  }

  if (bySize.length > limit) {
    bySize.length = limit;
  }
  ...
  var heaviestPages = pages.sort(function(page, thatPage) {
    return thatPage.yslow.pageWeight.v - page.yslow.pageWeight.v;
  });

  var lowestScoringPages = pages.slice();

  lowestScoringPages.sort(function(page, thatPage) {
    return page.score - thatPage.score;
  });

  if (lowestScoringPages.length > limit) {
    lowestScoringPages.length = limit;
  }

  if (heaviestPages.length > limit) {
    heaviestPages.length = limit;
  }
