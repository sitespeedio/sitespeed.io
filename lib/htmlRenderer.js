...
HTMLRenderer.prototype.renderToplists = function(assets, pages, domains, cb) {
  ...
  var assetsBySize = toplistUtil.getAssetsBySize(assets.slice(), limit);
  var largestImages = toplistUtil.getLargestImages(assets.slice(), limit);
  var biggestDiffAssets = toplistUtil.getLargestDiffBetweenLastModAndCache(assets.slice(), limit);
  var heaviestPages = toplistUtil.getLargestPages(pages.slice(), limit);
  var lowestScoringPages = toplistUtil.getLowestScoringPages(pages.slice(), limit);
