...
HTMLRenderer.prototype.renderSites = function(sitesAggregates, cb) {
  ...
  Object.keys(sitesAggregates).forEach(function(site) {
    ...
    Object.keys(sitesAggregates[site]).forEach(function(key) {
      flattenAggregates = flattenAggregates.concat(sitesAggregates[site][key])
    });

    sitesAndAggregates.push({
      'site': site,
      'aggregates': flattenAggregates.filter(function(box) {
