...
function InfluxdbCollector(config) {
  this.config = config;
  this.namespace = this.config.infludbNamespace;
...
InfluxdbCollector.prototype._getMeta = function(pages) {
  ...
  if (pages[0].har) {
    pages[0].har.forEach(function(har) {
      ...
      meta += 'meta_' + har.log.browser.name.toLowerCase() + '_version,namespace=' + namespace +  + ' value=' + version + self.timeStamp;
    });
  }
...
InfluxdbCollector.prototype._getPageMetricsStats = function(page, urlKey) {
  ...
  if (this.config.graphiteData.indexOf('pagemetrics') > -1 || this.config.graphiteData.indexOf(
    ...
    if (page.yslow) {
      ...
      statistics += 'page_yslow_timeSinceLastModification,namespace=' + self.namespace + '.' + urlKey + ' value=' + page.yslow.timeSinceLastModification
        .v + self.timeStamp;
    ...
    domains.forEach(function(domain) {
      timings.forEach(function(timing) {
        values.forEach(function(value) {
          ...
          stats += 'domains_timings_+' + timing + '_' + value + ',type=summary,hostname=' + hostname.split('.').join('_')  + ',domain=' + domain.domain.split('.').join('_') +
            ' value=' +util.getStatisticsObject(domain[timing].stats, 0)[value] + self.timeStamp;
        });
      });
      // and total time spent downloading
      stats += 'domains_accumulatedTime,type=summary,namespace='.self.namespace + ',hostname=' + hostname.split('.').join('_')  + ',domain=' + domain.domain.split('.').join('_') + ' value=' + domain.accumulatedTime + self.timeStamp;
...
//            ' ' + domain.size[size] + self.timeStamp;
          stats += 'domain_size' + ',type=summary,namespace=' + self.namespace + ',hostname=' + hostname.split('.').join('_')  + 
          ',domain_size=' + domain.domain.split('.').join('_') + size + 
          +' value=' + domain.size[size] + self.timeStamp;
...
InfluxdbCollector.prototype._getSummaryStats = function(aggregates, hostname, noOfPages) {
  ...
  aggregates.forEach(function(aggregate) {
    values.forEach(function(value) {
      ...
      if (aggregate.id.indexOf('WPT') > -1) {
        statistics += self.namespace + '.summary.' + hostname.split('.').join('_')  + '.' + aggregate.type + '.wpt.' +
      ...
      // catch navigation timings and separate them from the rest of the timings
        ...
        statistics += 'nagivationtiming' + aggregate.id + ',type=summary,namespace=' +self.namespace + ',hostname=' + hostname.split('.').join('_')  +
        + ' value=' + aggregate.stats[value] + self.timeStamp;
      } else {
        statistics += aggregate.type + '_'+aggregate.id + '_' + value + ',type=summary,hostname=' + hostname.split('.').join('_')  +
          ' value='  + aggregate.stats[value] + self.timeStamp;
      }
    });
  });
  ...
  statistics += 'runsPerBrowser,type=summary,hostname=' + hostname.split('.').join('_')  + ' value=' + this.config.no + this.timeStamp;

  // and number of tested pages per
  statistics += 'noOfPages,type=summary,hostname=' + hostname.split('.').join('_')  + ' value=' + noOfPages + this.timeStamp;
