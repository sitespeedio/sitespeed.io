        ...
        Object.keys(page[type]).forEach(function(timing) {
          statsWeWillPush.forEach(function(val) {
            ...
            if (self.config.supportedBrowsers.indexOf(timing) < 0) {
              statistics += self.namespace + '_' + urlKey + '_' + type + '_' + timing +
                '_' + val + ' value=' + page[type][timing][val].v + self.timeStamp;
            }
          });
        });
        ...
        Object.keys(page[type]).forEach(function(browser) {
          if (self.config.supportedBrowsers.indexOf(browser) > -1) {
            Object.keys(page[type][browser]).forEach(function(timing) {
              statsWeWillPush.forEach(function(val) {
                statistics += self.namespace + '_' + urlKey + '_' + type + '_' +
                  browser + '_' + timing + '_' + val +  ' value=' + page[type][browser][timing][val].v + self.timeStamp;
              });
            });
          }
        });
        ...
        har.log.entries.forEach(function(entry) {
          ...
          if (entry.timings) {
            timings.forEach(function(timing) {
              ...
              stats += self.namespace + '_' + urlKey + '_requests_' + assetURL + '_timings_' + timing  + ' value=' +  entry.timings[timing] + timeStamp;
            });
            stats += self.namespace + '_' + urlKey + '_requests_' + assetURL + '_timings_total ' + ' value=' + total +
              timeStamp;
          }
          ...
          stats += self.namespace + '_' + urlKey + '_requests_' + assetURL + '_type_' +
            util.getContentType(entry.response.content.mimeType) + '_size value=' + entry.response.content.size + ' ' + self.timeStamp;
    ...
    domains.forEach(function(domain) {
      timings.forEach(function(timing) {
        values.forEach(function(value) {
          ...
          stats += 'summary_domains_timings_' + timing + '_' + value + ',hostname=' + hostname.split('.').join('_')  + ',domain=' + domain.domain.split('.').join('_') +
            ' value=' +util.getStatisticsObject(domain[timing].stats, 0)[value] + self.timeStamp;
        });
      });
      // and total time spent downloading
      stats += 'summary_domains_accumulatedTime,namespace=' + self.namespace + ',hostname=' + hostname.split('.').join('_')  + ',domain=' + domain.domain.split('.').join('_') + ' value=' + domain.accumulatedTime + self.timeStamp;

      // the number of requests
      stats += "summary_domains_requests,namespace=" + self.namespace + ',hostname=' + hostname.split('.').join('_')  + ",domain=" + domain.domain.split('.').join('_') + ' value=' + domain.count + self.timeStamp;
  ...
  aggregates.forEach(function(aggregate) {
    values.forEach(function(value) {
      ...
      // catch navigation timings and separate them from the rest of the timings
        ...
        statistics += 'summary_nagivationtiming_' + aggregate.id + ',namespace=' +self.namespace + ',hostname=' + hostname.split('.').join('_')  + ' value=' + aggregate.stats[value] + self.timeStamp;
      } else {
        statistics += "summary_" + aggregate.type + '_'+aggregate.id + '_' + value + ',hostname=' + hostname.split('.').join('_')  +
