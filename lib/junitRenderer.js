...
function renderXMLFile(xml, fileName, cb) {
  fs.writeFile(path.join(config.run.absResultDir, fileName), xml, function(err) {
    if (err) {
      log.log('error', 'Couldn\'t write JUnit xml files' + config.run.absResultDir);
      throw err;
    }
    cb();
  });
...
JUnitRenderer.prototype.renderAfterFullAnalyse = function(cb) {
  ...
  async.parallel({
      renderRules: function(callback) {
        renderXMLFile(rulesXML,'junit.xml',callback);
      },
      renderTimings: function(callback) {
        renderXMLFile(timingXML,'junit-timings.xml',callback);
      }
    },
    function(err, results) {
      if (!err) {
        log.log('info', 'Wrote JUnit result to ' + config.run.absResultDir);
      }
      cb();
    });
};
