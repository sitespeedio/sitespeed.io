...
module.exports = {
  ...
  addToAggregate(data, group, url, storageManager) {
    ...
    if (this.groups[group] === undefined) {
      ...
      this.slowestAssetsByGroup[group] = {};
    }
    ...
    for (const asset of data.assets) {
      ...
      if (this.slowestAssets.items) {
        this.slowestAssets.add(asset, page);
      } else {
        this.slowestAssets = new AssetsBySpeed(10, asset, page);
      }

      if (this.slowestAssetsByGroup[group].items) {
        this.slowestAssetsByGroup[group].add(asset, page);
      } else {
        this.slowestAssetsByGroup[group] = new AssetsBySpeed(10, asset, page);
      }
