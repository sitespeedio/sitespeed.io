...
module.exports = {
  ...
  addToAggregate(browsertimeRunData, url) {
    ...
    forEach(timings, (timing) => {
      if (browsertimeRunData.timings[timing]) {
        statsHelpers.pushDomainStats(this.statsPerType, this.groups[group], timing, browsertimeRunData.timings[timing]);
      }
    })

    forEach(browsertimeRunData.timings.navigationTiming, (value, name) => {
      statsHelpers.pushDomainStats(this.statsPerType, this.groups[group], ['navigationTiming', name], value);
    });

    // pick up one level of custom metrics
    forEach(browsertimeRunData.custom, (value, name) => {
      statsHelpers.pushDomainStats(this.statsPerType, this.groups[group], ['custom', name], value);
    });

    forEach(browsertimeRunData.timings.timings, (value, name) => {
      statsHelpers.pushDomainStats(this.statsPerType, this.groups[group], ['timings', name], value);
    });

    forEach(browsertimeRunData.timings.userTimings.marks, (timing) => {
      statsHelpers.pushDomainStats(this.statsPerType, this.groups[group], ['userTimings', 'marks', timing.name], timing.startTime);
    });

    forEach(browsertimeRunData.timings.userTimings.measures, (timing) => {
      statsHelpers.pushDomainStats(this.statsPerType, this.groups[group], ['userTimings', 'measures', timing.name], timing.duration);
    });

    forEach(browsertimeRunData.visualMetrics, (value, name) => {
      if (name !== 'VisualProgress') {
        statsHelpers.pushDomainStats(this.statsPerType, this.groups[group], ['visualMetrics', name], value);
      }
    });
