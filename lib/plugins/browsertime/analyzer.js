...
module.exports = {
  analyzeUrl(url, options) {
    ...
    // There's a bug in Geckodriver not handling window sizes
    // https://github.com/mozilla/geckodriver/issues/113
    // removing setting the viewport lets everything work until
    // it is fixed.
    if (options.browsertime.browser === 'firefox') {
      options.browsertime.viewPort = undefined;
    }
