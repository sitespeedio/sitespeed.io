  ...
  return Promise.reduce(
    ...
    (results, script) =>
      browserScripts
        ...
        .then(scripts => Object.assign(results, scripts)),
    {}
  );
}
...
module.exports = {
  analyzeUrl(url, options) {
    const btOptions = Object.assign({}, defaultBrowsertimeOptions, options);
    Object.assign(btOptions, { verbose: options.verbose });

    // set mobile options
    if (options.mobile) {
      btOptions.viewPort = '360x640';
      if (btOptions.browser === 'chrome') {
        btOptions.chrome = Object.assign(
          {},
          btOptions.chrome,
    ...
    if (btOptions.script) {
      ...
      scriptsByCategory = Promise.join(
        ...
        (scriptsByCategory, userScripts) =>
          Object.assign(scriptsByCategory, userScripts)
      );
    }
