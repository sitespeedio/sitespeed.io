  ...
  addToAggregate(browsertimeRunData, group) {
    ...
    if (browsertimeRunData.visualMetrics) {
      for (const [name, value] of Object.entries(
        ...
        // Skip FrameDiffs (an array of per-frame objects) and
        // FrameInstability (a summary object) — not numbers, nothing
        // to summarize. Same exclusions as browsertime's collector.
        if (
          !name.includes('Progress') &&
          name !== 'FrameDiffs' &&
          name !== 'FrameInstability' &&
          value !== null
        ) {
          pushGroupStats(
            this.statsPerType,
