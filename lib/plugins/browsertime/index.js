            ...
            .tap((results) => {
              ...
              results.browserScripts.forEach((run, runIndex) => {
                ...
                if (results.har) {
                  ...
                  // always add firstContentfulPaint and in the future pickup
                  // whatever paint time we have?
                  if (run.timings.paintTiming) {
                    run.har.log.pages[0].pageTimings._firstContentfulPaint = run.timings.paintTiming["first-contentful-paint"];
                    results.har.log.pages[runIndex].pageTimings._firstContentfulPaint = run.timings.paintTiming["first-contentful-paint"];
                  }

                  if (run.timings.pageTimings) {
                    run.har.log.pages[0].pageTimings._domInteractiveTime = run.timings.pageTimings.domInteractiveTime;
                    results.har.log.pages[runIndex].pageTimings._domInteractiveTime = run.timings.pageTimings.domInteractiveTime;

                    run.har.log.pages[0].pageTimings._domContentLoadedTime = run.timings.pageTimings.domContentLoadedTime;
                    results.har.log.pages[runIndex].pageTimings._domContentLoadedTime = run.timings.pageTimings.domContentLoadedTime;
                  }
                }
