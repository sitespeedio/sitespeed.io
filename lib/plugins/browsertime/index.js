...
module.exports = {
  ...
  processMessage(message, queue) {
    ...
    function processCoachOutput(url, results) {
      const pageSummaries = harApi.getPagesFromHar(results.har);

      return Promise.resolve(results.browsertimeData).each((run, runIndex) => {
        // make sure to get the right page summary
        return harApi.runAdvice([pageSummaries[runIndex]], harApi.getAllAdvice(), {})
          .then((harResult) => {
