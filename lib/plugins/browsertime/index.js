  ...
  processMessage(message, queue) {
    ...
    function processCoachOutput(url, results) {
      ...
      return Promise.resolve(results.browserScripts).each((run, runIndex) => {
        ...
        if (Object.keys(run.coach.coachAdvice.errors).length > 0) {
          log.error('The %s generated the following errors in the coach %:2j', url, run.coach.coachAdvice.errors);
           queue.postMessage(make('error', 'The coach got the following error:' + JSON.stringify(run.coach.coachAdvice.errors), {url: url}));
        }
        ...
        return api.runHarAdvice(myHar)
          .then((harResult) => api.merge(run.coach.coachAdvice, harResult))
