...
const DEFAULT_METRICS_PAGE_SUMMARY = require('./defaultMetricsPageSummary');
const DEFAULT_METRICS_SUMMARY = require('./defaultMetricsSummary');
    ...
    switch (message.type) {
      ...
      case 'url': {
        ...
        for (let run of result.browserScripts) {
          ...
          // --browsertime.chrome.passOnChromeTrace
          if (
            options.chrome &&
            options.chrome.timeline &&
            options.chrome.passOnChromeTrace
          ) {
