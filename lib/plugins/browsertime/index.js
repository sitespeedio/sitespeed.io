...
module.exports = {
  ...
  processMessage(message, queue) {
    ...
    switch (message.type) {
      ...
      case 'browsertime.script': {
        if (message.data.name && message.data.category && message.data.script) {
          this.pluginScripts.push(message.data);
        } else {
          log.error(
            'Got wrong formated browsertime.script. You need name, category and script:' +
              message.data
          );
        }
      ...
      case 'url': {
        ...
        return this.storageManager
          ...
          .then(() =>
            analyzer.analyzeUrl(url, this.options, this.pluginScripts)
          )
          .tap(results => {
            log.verbose(
