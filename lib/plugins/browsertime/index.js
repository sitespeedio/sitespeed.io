    ...
    switch (message.type) {
      ...
      case 'url': {
        ...
        try {
          ...
          if (options.screenshot && this.allOptions.copyLatestFilesToBase) {
            const baseDir = this.storageManager.getBaseDir();
            // Hack to get out of the date dir
            const newPath = path.resolve(baseDir, '..');
            let i = 0;
            for (let r of result) {
              const fullPath = path.join(baseDir, r.files.screenshot[0][i]);
              i++;
              ...
              const name = graphiteUtil.getURLAndGroup(
                ...
                group,
                r.info.url,
                this.allOptions.graphite.includeQueryParams,
                this.allAlias
              );
