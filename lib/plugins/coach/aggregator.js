...
module.exports = {
  ...
  addToAggregate(coachData, url) {
    ...
    statsHelpers.pushDomainStats(this.statsPerCategory, this.groups[group], 'score', coachData.advice.score);
    ...
    forEach(coachData.advice, (category, categoryName) => {
      ...
      statsHelpers.pushDomainStats(this.statsPerCategory, this.groups[group], [categoryName, 'score'], category.score);

      forEach(category.adviceList, (advice, adviceName) => {
        statsHelpers.pushDomainStats(this.statsPerCategory, this.groups[group], [categoryName, adviceName], advice.score);
      });
    });
  },
  summarize() {
