...
module.exports = {
  stats: {},
  groups: {},
  addToAggregate(pageSummary, url) {
    ...
    pageSummary.forEach(function(summary) {
      ...
      METRIC_NAMES.forEach(function(metric) {
        statsHelpers.pushDomainStats(stats, groups[group], metric, summary[metric]);
      });

      Object.keys(summary.contentTypes).forEach(function(contentType) {
        METRIC_NAMES.forEach(function(metric) {
          statsHelpers.pushDomainStats(stats,groups[group], 'contentTypes.' + contentType + '.' + metric, summary.contentTypes[contentType][metric]);
        });
      })

      Object.keys(summary.responseCodes).forEach(function(responseCode) {
        statsHelpers.pushDomainStats(stats, groups[group], 'responseCodes.' + responseCode, summary.responseCodes[responseCode]);
      });

      // extras for firstParty vs third
      if (summary.firstParty.requests) {
        METRIC_NAMES.forEach(function(metric) {
          statsHelpers.pushDomainStats(stats, groups[group], 'firstParty' + '.' + metric, summary.firstParty[metric]);
          statsHelpers.pushDomainStats(stats, groups[group], 'thirdParty' + '.' + metric, summary.thirdParty[metric]);
        })
      }

      // Add the total amount of domains on this page
      statsHelpers.pushDomainStats(stats, groups[group], 'domains', (Object.keys(summary.domains)).length);

      forEach(summary.assets, (asset) => {
        statsHelpers.pushDomainStats(stats, groups[group], 'expireStats', asset.expires);
        statsHelpers.pushDomainStats(stats, groups[group], 'lastModifiedStats', asset.timeSinceLastModified);
      });
