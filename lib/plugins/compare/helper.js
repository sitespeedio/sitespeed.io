...
// Hodges-Lehmann shift estimate with a confidence interval: the median of
// all pairwise current - baseline differences, with CI bounds taken as
// order statistics of those sorted differences. The rank offset comes from
// the normal approximation of the Mann-Whitney statistic (fine at the 20+
// iterations the plugin recommends; with heavy ties the interval is
// slightly conservative). Answers "how big is the shift and how sure are
// we" where the p-value only answers "how consistent is it".
export function hodgesLehmann(current, baseline, confidence = 0.95) {
  ...
  const mid = total >> 1;
  const estimate =
    total % 2 === 1 ? diffs[mid] : (diffs[mid - 1] + diffs[mid]) / 2;

  // Two-sided z for the requested confidence level; only 0.95 and 0.99 are
  // ever asked for today, so a lookup beats pulling in an inverse-normal.
  const z = confidence === 0.99 ? 2.576 : 1.96;
  const k = Math.max(
    1,
    Math.floor(total / 2 - z * Math.sqrt((n * m * (n + m + 1)) / 12))
  );
