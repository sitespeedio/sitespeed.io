...
const flatten = require('../../support/flattenMessage'),
  util = require('util'),
  elasticsearchUtil = require('../../support/tsdbUtil'),
...
function keyPathFromMessage(message, options, includeQueryParams) {
  ...
  if (message.type.match(/(^pagexray|^coach|^browsertime|^largestassets|^slowestassets|^aggregateassets|^domains)/)) {
    ...
    let connectivity = elasticsearchUtil.getConnectivity(options);
  ...
  if (message.url) {
    typeParts.splice(1, 0, elasticsearchUtil.getURLAndGroup(options, message.group, message.url, includeQueryParams));
  } else if (message.group) {
    // add the group of the summary message
    typeParts.splice(1, 0, elasticsearchUtil.toSafeKey(message.group));
  }

  return typeParts.join('.');
}

module.exports = ElasticSearchDataGenerator;
