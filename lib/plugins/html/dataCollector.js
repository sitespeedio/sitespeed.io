...
export class DataCollector {
  ...
  addErrorForUrl(url, source, data, alias) {
    ...
    setProperty(this.urlPages[url], 'errors', errors);
  }
  ...
  addDataForUrl(url, typePath, data, runIndex, alias) {
    ...
    if (runIndex === undefined) {
      setProperty(this.urlPages[url].data, typePath, data);
    } else {
      ...
      setProperty(runData.data, typePath, data);
  ...
  addSummary(name, data) {
    if (this.summaryPages[name]) {
      ...
      setProperty(this.summaryPages, name, merge({}, data));
    }
  }
