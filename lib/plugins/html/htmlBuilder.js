...
const fs = require('fs'),
  ...
  log = require('intel').getLogger('plugin.html'),
...
class HTMLBuilder {
    ...
    const urlPageRenders = Promise.resolve(Object.keys(validPages))
      .map((url) => {
        ...
        if (!(isEmpty(runPages) || options.html.showAllWaterfallSummary)) {
          ...
          const har = get(runPages[0], 'data.browsertime.run.har');
          if (har) {
            summaryPageHAR = har;
          }
        }
  ...
  _renderUrlPage(url, name, locals) {
    locals = merge({
      ...
      menu: 'pages',
      pageTitle: `Summary for ${helpers.plural(this.options.browsertime.iterations,'run')} ${url} at ${this.timestamp}`,
      pageDescription: `Min, median and max values for all the runs testing ${url}`,
  ...
  _renderUrlRunPage(url, name, locals) {
    locals = merge({
      ...
      rootPath: this.storageManager.rootPathFromUrl(url),
      menu: 'pages',
      pageTitle: `Run ${(parseInt(name) + 1)} for ${url} at ${this.timestamp}`,
      pageDescription: 'Get all the metrics for that specific run.',
      headers: this.summary,
      version: packageInfo.version,
      timestamp: this.timestamp,
      h: helpers,
