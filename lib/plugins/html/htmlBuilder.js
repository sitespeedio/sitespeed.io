...
export class HTMLBuilder {
  ...
  async render(dataCollector) {
    ...
    // Compact per-page medians for the scorecard class list on the start
    // page. Missing metrics stay undefined so the template can degrade.
    const scorecardPages = Object.entries(validPages).map(([url, pageInfo]) => {
      const alias =
        (options.urlsMetaData &&
          options.urlsMetaData[url] &&
          options.urlsMetaData[url].urlAlias) ||
        get(pageInfo, 'data.browsertime.pageSummary.info.alias');
      return {
        url,
        displayName: alias || url,
        href: pageInfo.path + 'index.html',
        lcp: get(
          pageInfo,
          'data.browsertime.pageSummary.statistics.timings.largestContentfulPaint.renderTime.median'
        ),
        cls: get(
          pageInfo,
          'data.browsertime.pageSummary.statistics.pageinfo.cumulativeLayoutShift.median'
        ),
        tbt: get(
          pageInfo,
          'data.browsertime.pageSummary.statistics.cpu.longTasks.totalBlockingTime.median'
        ),
        speedIndex: get(
          pageInfo,
        ...
        transferSize: get(
          pageInfo,
          'data.pagexray.pageSummary.statistics.transferSize.median'
        ),
        coachScore: get(
          pageInfo,
          'data.coach.pageSummary.advice.performance.score'
        )
      };
    });
    ...
    this.summary.index = {
      ...
      boxes: summaryBoxes.filter(Boolean),
      scorecardPages
    };

    this.summary.detailed = {
      pageTitle: `Detailed summary for ${name} tested ${helpers.plural(
        nTestedPages,
        'page'
      )} at ${timestamp}`,
      pageDescription:
        'Every collected metric for the tested pages, grouped by concern, with the spread across all runs.',
    ...
    this.summary.assets = {
      pageTitle: `The heaviest assets for ${name} tested at ${timestamp}`,
      pageDescription: 'A list of the heaviest assets for the analysed pages.'
    };
    ...
    if (options.multi && options.html.showScript) {
      ...
      this.summary.scripts = {
        pageTitle: `Scripts used to run the analysis`,
        pageDescription:
          'The user scripts that drove the test runs in this report.',
        scripts
      };
    }
    ...
    this.summary.help = {
      pageTitle: 'Definitions and help for all metrics',
      pageDescription:
        'What every metric in the report means and how to read the numbers.',
      coach: coachData
    };
    ...
    for (let url of Object.keys(validPages)) {
      ...
      browser = usingBrowsertime
        ? {
            ...
            preferences:
              pageInfo.data.browsertime.pageSummary.info.browser.preferences,
            binary: pageInfo.data.browsertime.pageSummary.info.browser.binary,
            binaryPath:
              pageInfo.data.browsertime.pageSummary.info.browser.binaryPath,
            profile: pageInfo.data.browsertime.pageSummary.info.browser.profile,
            mobileEmulation:
              pageInfo.data.browsertime.pageSummary.info.browser
                .mobileEmulation,
            extensions:
              pageInfo.data.browsertime.pageSummary.info.browser.extensions,
            androidPackage:
              pageInfo.data.browsertime.pageSummary.info.browser.androidPackage,
            androidActivity:
              pageInfo.data.browsertime.pageSummary.info.browser
                .androidActivity,
