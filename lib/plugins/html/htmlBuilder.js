    ...
    const urlPageRenders = Promise.resolve(Object.keys(validPages)).map(url => {
      ...
      */
      ...
      for (const summary of pageSummaries) {
        pugs[summary.id] = renderer.renderTemplate(summary.id, {
          pageInfo,
          h: helpers
        });
      ...
      return this._renderUrlPage(url, 'index', {
        ...
        Promise.resolve(Object.keys(runPages)).map(runIndex => {
          ...
          for (const run of pageRuns) {
            pugs[run.id] = renderer.renderTemplate(run.id, {
              pageInfo,
              h: helpers
            });
            locals = merge({ pugs }, locals);
          ...
          return this._renderUrlRunPage(url, runIndex, {
            daurl: url,
            daurlAlias,
            runIndex,
            pageInfo: pageInfo,
            ...
            menu: 'pages',
            pageTitle: `Run ${parseInt(name) + 1} for ${url} at ${
              runTimestamp
            }`,
