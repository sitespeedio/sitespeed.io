...
class HTMLBuilder {
  ...
  addType(type, name) {
    switch (type) {
      case 'run': {
        this.pageRuns.push(name);
        break;
      }
      case 'pageSummary': {
        this.pageSummaries.push(name);
        break;
      }
      case 'summary': {
  ...
  _renderUrlPage(url, name, locals) {
    ...
    for (var summaries of this.pageSummaries) {
      pugs.pugs[summaries] = renderer.renderTemplate(summaries, locals);
  ...
  _renderUrlRunPage(url, name, locals) {
    ...
    for (var run of this.pageRuns) {
      pugs.pugs[run] = renderer.renderTemplate(run, locals);
      locals = merge(pugs, locals);
    }
