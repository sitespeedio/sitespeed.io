...
class HTMLBuilder {
  ...
  _renderUrlPage(url, name, locals) {
    const summaryTimestamp = locals.pageInfo.data.browsertime.pageSummary.timestamp;
    locals = merge({
      JSON: JSON,
      ...
      pageDescription: `${metricHelper.getMetricsFromPageSummary(locals)} collected by sitespeed.io ${packageInfo.version}`,
  ...
  _renderUrlRunPage(url, name, locals) {
    const runTimestamp = locals.pageInfo.data.browsertime.run.timestamp;
    locals = merge({
      urlLink: './index.html',
      JSON: JSON,
      rootPath: this.storageManager.rootPathFromUrl(url),
      ...
      pageDescription: `${metricHelper.getMetricsFromRun(locals)} collected by sitespeed.io ${packageInfo.version}`,
      headers: this.summary,
      version: packageInfo.version,
