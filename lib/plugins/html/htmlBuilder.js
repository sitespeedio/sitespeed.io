...
class HTMLBuilder {
  ...
  _renderUrlPage(url, name, locals) {
    ...
    if (this.gzip) {
      return zlib
        .gzipAsync(
          Buffer.from(renderer.renderTemplate('url/' + name, locals)),
          {
            level: 1
          }
        )
        .then(gziped =>
          this.storageManager.writeHtmlForUrl(gziped, name + '.html.gz', url)
        );
    } else {
  ...
  _renderUrlRunPage(url, name, locals) {
    ...
    if (this.gzip) {
      return zlib
        .gzipAsync(Buffer.from(renderer.renderTemplate('url/run', locals)), {
          level: 1
        })
        .then(gziped =>
          this.storageManager.writeHtmlForUrl(gziped, name + '.html.gz', url)
  ...
  _renderSummaryPage(name, locals) {
    ...
    if (this.gzip) {
      return zlib
        .gzipAsync(Buffer.from(renderer.renderTemplate(name, locals)), {
          level: 1
        })
        .then(gziped =>
          this.storageManager.writeHtml(gziped, name + '.html.gz')
        );
    } else {
