...
export default class HTMLPlugin extends SitespeedioPlugin {
  ...
  open(context, options) {
    ...
    this.maxAssets = get(options, 'maxAssets', 100);
    this.alias = {};
      ...
      switch (message.type) {
        ...
        case 'aggregateassets.summary': {
          if (message.group === 'total') {
            ...
            // The overview strip on the Assets page is computed over ALL
            // unique assets in the run, not just the top list, so the
            // totals stay truthful when the table is capped by --maxAssets.
            let totalTransferSize = 0;
            let totalContentSize = 0;
            let biggestAsset;
            const typeCounts = {};
            for (const asset of assetList) {
              totalTransferSize += asset.transferSize || 0;
              totalContentSize += asset.size || 0;
              const type = asset.type || 'other';
              typeCounts[type] = (typeCounts[type] || 0) + 1;
              const weight = asset.transferSize || asset.size || 0;
              if (
                !biggestAsset ||
                weight > (biggestAsset.transferSize || biggestAsset.size || 0)
              ) {
                biggestAsset = asset;
              }
            }

            // Select the survivors of the --maxAssets cap by the same
            // weight rule the page sorts by (wire bytes, content size as
            // fallback) — selecting by request count let the heaviest
            // assets fall off the page entirely when they were only
            // requested once.
            const count = this.maxAssets,
              ...
              topAssets = assetList
                .sort(
                  (a, b) =>
                    (b.transferSize || b.size || 0) -
                    (a.transferSize || a.size || 0)
                )
                .splice(0, count);
