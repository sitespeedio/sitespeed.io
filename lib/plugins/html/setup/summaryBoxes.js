...
function box(name, metric, score, helper) {
  return {
    name,
    score,
...
module.exports = function(data) {
  ...
  // coach
  if (coach) {
    boxes.push(box('Overall score', coach.summary.score, coach.summary.score[metricForScore]));
    boxes.push(box('Performance score', coach.summary.performance.score, coach.summary.performance.score[metricForScore]));
    boxes.push(box('Accessibility score', coach.summary.accessibility.score, coach.summary.accessibility.score[metricForScore]));
    boxes.push(box('Best Practice score', coach.summary.bestpractice.score, coach.summary.bestpractice.score[metricForScore]));
    boxes.push(box('Fast Render advice', coach.summary.performance.fastRender, coach.summary.performance.fastRender[metricForScore]));
    boxes.push(box('Avoid scaling images advice', coach.summary.performance.avoidScalingImages, coach.summary.performance.avoidScalingImages[metricForScore]));
    boxes.push(box('Compress assets advice', coach.summary.performance.compressAssets, coach.summary.performance.compressAssets[metricForScore]));
  }

  if (pagexray && coach) {
    boxes.push(box('Total size (transfer)', pagexray.summary.transferSize, coach.summary.performance.pageSize[metricForScore], h.size.format));

    boxes.push(box('Image size (transfer)', pagexray.summary.contentTypes.image.transferSize, coach.summary.performance.imageSize[metricForScore], h.size.format));

    boxes.push(box('Javascript size (transfer)', pagexray.summary.contentTypes.javascript.transferSize, coach.summary.performance.javascriptSize[metricForScore], h.size.format));

    boxes.push(box('Optimal CSS size (transfer)', pagexray.summary.contentTypes.css.transferSize, coach.summary.performance.optimalCssSize[metricForScore], h.size.format));
