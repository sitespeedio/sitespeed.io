...
import { getAnnotationMessage } from '../../support/annotationsHelper.js';
...
export function sendV2(
  ...
  if (options.influxdb.tags) {
    tags.push(options.influxdb.tags);
  }
  const influxDBTags = tags.join(',');
  const postData = `annotations,${influxDBTags} text="${message}" ${timestamp}`;
  const postOptions = {
