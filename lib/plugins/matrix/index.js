    ...
    switch (message.type) {
      ...
      case 's3.finished': {
        if (this.waitForUpload && options.messages.indexOf('budget') > -1) {
          ...
          this.waitForUpload &&
          options.messages.indexOf('done') > -1
        ) {
          const room = get(options, 'rooms.done', options.room);
          const message = `<a href="${this.resultUrls.reportSummaryUrl() +
            '/index.html'}">Result</a>`;
          await send(options.host, room, options.accessToken, message);
        }
        break;
      }
    }
