...
export default class OpenSearchPlugin extends SitespeedioPlugin {
  ...
  open(context, options) {
    ...
    super.log('Initilizaing the opensearch plugin connection');
  ...
  async processMessage(message, queue) {
    if (message.type === 'sitespeedio.setup') {
      ...
      super.log(`SiteSpeedio setup message:: ${message.type}`);
      ...
      try {
        ...
        super.log(`SiteSpeedio metrics data:: ${message.data}`);
      } catch (error) {
