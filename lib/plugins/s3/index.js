...
function createS3Client(s3Options) {
  ...
  */
  ...
  const extraParams = Object.keys(this.s3Options.extraClientParams);
  for (let key of extraParams) {
    awsS3Client[key] = this.s3Options.extraClientParams[key];
  }
...
module.exports = {
  ...
  processMessage(message, queue) {
    ...
    if (message.type === 'html.finished') {
      ...
      const extraParams = Object.keys(this.s3Options.extraParams);
      for (let key of extraParams) {
        params.s3Params[key] = this.s3Options.extraParams[key];
      }
