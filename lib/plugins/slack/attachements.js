...
module.exports = function(dataCollector, resultUrls, slackOptions) {
  const attachments = [];

  const resultsPerUrl = dataCollector.getURLs();
  ...
  for (let url of Object.keys(resultsPerUrl)) {
    ...
    if (resultUrls.hasBaseUrl()) {
      attachement['image_url'] =
        resultUrls.absoluteSummaryPageUrl(url) + 'data/screenshots/0.png';
    }
