...
module.exports = function(dataCollection, slackOptions, resultBaseURL, storageManager) {
  const attachments = [];
  ...
  for (let url of Object.keys(resultsPerUrl)) {
    ...
    let message = '';
    if (resultBaseURL) {
      const baseDir = storageManager.getRelativeBaseDir();
      const pagePath = storageManager.pathFromRootToPageDir(url);
      message = ` (<${resultBaseURL}/${baseDir}/${pagePath}|result>)`;
    }
