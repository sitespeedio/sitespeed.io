...
module.exports = function(dataCollection, slackOptions) {
  ...
  for (let url of Object.keys(resultsPerUrl)) {
    ...
    let color = 'danger';
      ...
      const limitMetric = metrics[slackOption.limitMetric];
      if (limitMetric && slackOption.limitMetric === 'coachScore') {
        if (limitMetric.metric > slackOption.limitError) {
          color = 'good';
        }
        else if (limitMetric.metric > slackOption.limitWarning) {
          color = 'warning';
        }
      }
