...
module.exports = {
  get(dataCollection, options) {
    ...
    for (let url of Object.keys(resultsPerUrl)) {
      ...
      const limitMetric = metrics[options.limitMetric];
      if (limitMetric && options.limitMetric === 'coach') {
        if (limitMetric.metric > options.limitError) {
          color = 'good';
        }
        else if (limitMetric.metric > options.limitWarning) {
          color = 'warning';
        }
      }
      // SpeeddIndex/firstVisualChange
      else if (limitMetric) {
        if (limitMetric.metric < options.limitError) {
          color = 'good';
        }
