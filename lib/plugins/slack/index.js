...
const log = require('intel').getLogger('plugin.slack');
...
module.exports = {
  ...
  close(options) {
    ...
    if (['summary', 'all', 'error'].includes(type)) {
      const sum = getSummary(this.data, pageErrors, this.storageManager, this.context, options);
    ...
    if (['url', 'all', 'error'].includes(type)) {
      attachments = getAttachments(this.data, options.resultBaseURL, this.storageManager, slackOptions);
    }
