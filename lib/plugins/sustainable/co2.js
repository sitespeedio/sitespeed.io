...
function sumTransferOfDomains(domainCO2) {
  ...
  return reduce(domainCO2, reducer);
}
...
function dirtiestResources(pageXray, green) {
  // find the top 10 resources by carbon from transfer size and energy choi
}
*/

function perParty(pageXray, green) {
  let firstParty = 0;
  let thirdParty = 0;
  ...
  for (let d of Object.keys(pageXray.domains)) {
    if (!d.match(firstPartyRegEx)) {
      thirdParty += CO2PerByte(pageXray.domains[d].transferSize, green);
    } else {
      firstParty += CO2PerByte(pageXray.domains[d].transferSize, green);
    }
  }
