...
function getCO2PerParty(pageXray, green) {
  let firstParty = 0;
  let thirdParty = 0;
  ...
  for (let d of Object.keys(pageXray.domains)) {
    if (!d.match(firstPartyRegEx)) {
      thirdParty += getCO2PerByte(pageXray.domains[d].transferSize, green);
    } else {
      firstParty += getCO2PerByte(pageXray.domains[d].transferSize, green);
    }
  }
