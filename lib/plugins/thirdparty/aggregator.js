...
module.exports = {
  ...
  addToAggregate(
    ...
    for (let tool of Object.keys(downloadTimePerTool)) {
      statsHelpers.pushStats(
        this.downloadTimePerTool[url],
        tool.replace(/\./g, '_'),
        downloadTimePerTool[tool]
      );
    }
  },
  summarize() {
    ...
    for (let url of Object.keys(this.urls)) {
      ...
      for (let tool of Object.keys(this.downloadTimePerTool[url])) {
        statsHelpers.setStatsSummary(
          summary[url],
          'tool.' + tool + '.downloadTime',
          this.downloadTimePerTool[url][tool]
        );
      }
