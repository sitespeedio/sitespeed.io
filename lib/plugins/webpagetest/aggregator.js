...
const forEach = require('lodash.foreach'),
  log = require('intel').getLogger('plugins.webpagetest.aggregator'),
...
module.exports = {
  ...
  addToAggregate(group, wptData, connectivity, location) {
    ...
    this.customGroups[group] = this.customGroups[group] || {}
...
// pushGroupStats(target, domainTarget, path, data) {
          ...
          // in xmlResult.php : 
          // <custom>
          //  <value>gifNumber</value>
          // </custom>
          // <gifNumber>7</gifNumber>

          // log.info(metricName, viewData[metricName])
        })
      });
    });
  },
  summarize() {
    const summary = {
      groups: {
        total: {
          ...
          customMetrics: this.summarizePerCustomMetrics(this.customStats)
        }
      }
    };
    ...
    for (let group of Object.keys(this.customGroups)) {
      if (!summary.groups[group]) summary.groups[group] = {};
      summary.groups[group].customMetrics = this.summarizePerCustomMetrics(this.customGroups[group]);
    }
    return summary;
  },
  ...
  summarizePerCustomMetrics(type) {
    const summary = {};
    forEach(type, (view, viewName) =>
      forEach(view, (contentType, contentTypeName) =>
        forEach(contentType, (stats, propertyName) =>
          statsHelpers.setStatsSummary(summary, [viewName, 'customMetrics', propertyName], stats))));
    return summary;
  }
