...
let path = require('path'),
  ...
  log = require('intel').getLogger('plugins.webpagetest'),
...
module.exports = {
  ...
  open(context, options) {
    ...
    if (!options.key) {
      ...
      if (host && host[2] === defaultWptHost) {
        log.error('webpagetest.key needs to be specified when using the public WebPageTest server')
        throw new Error('webpagetest.key needs to be specified when using the public WebPageTest server.');
      }
    }

    log.info('Used options :', this.options)
  ...
  processMessage(message, queue) {
    switch (message.type) {
      ...
      case 'error':
      {
        if(message.source === 'webpagetest') {
          log.error('webpagetest wrapper said', message.data, new Error(message.data))
        }
      }
    }
  },
