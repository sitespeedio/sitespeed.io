      ...
      case 'url':
        {
          ...
          return analyzer.analyzeUrl(url, this.options)
            .tap((result) => {
              queue.postMessage(make('webpagetest.har', result.har, {
                url
              }));
              ...
              aggregator.addToAggregate(url, result, connectivity, location);
            })
            .catch((err) => {
      ...
      case 'summarize':
        {
          ...
          if (summary && Object.keys(summary.groups).length > 0) {
            for (let group of Object.keys(summary.groups)) {
              queue.postMessage(make('webpagetest.summary', summary.groups[group].asset, {
                connectivity: aggregator.connectivity,
                location: aggregator.location,
                group
              }));

              queue.postMessage(make('webpagetest.summary', summary.groups[group].timing, {
                connectivity: aggregator.connectivity,
                location: aggregator.location,
                group
              }));
