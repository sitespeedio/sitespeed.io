...
SiteHTMLRenderer.prototype.renderHAR = function(url, pageData, cb) {
  ...
  this.config.browsertime.forEach(function(browser) {

    var hash = util.getFileName(url);

    try {

    console.log('Lets create the simplehar result');
    ...
    result.html = Array.isArray(result.html) ?
    ...
    result.html.forEach(function(har) {
      ...
      renderData.pageMeta = {
        ...
        'description': 'HAR waterfall graph.'
      };
      ...
      log.error('Could not generate HAR for ' + url + '  err:' + err);
    }
  });
};
