  ...
  async.series([
    ...
    function(err, results) {
      ...
      if (self.config.processJson) {
        var processJson = new AnalyzeOneSite(self.config);
      ...
      // else test one site
        ...
        var analyzeMultipleSites = new AnalyzeMultipleSites(self.config);
        analyzeMultipleSites.run(finishedCb);
      } else {
        var analyzeOneSite = new AnalyzeOneSite(self.config);
