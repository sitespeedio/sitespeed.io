...
function runDeprecatedOptionalFunction(objects, fN) {
  // NOTE: note slicing due to https://github.com/petkaantonov/bluebird/wiki/Optimization-killers#3-managing-arguments
  let args = new Array(arguments.length - 2);
  for (let i = 2; i < arguments.length; i++) {
    args[i - 2] = arguments[i];
  }
  return Promise.resolve(objects).filter(hasFunctionFilter(fN)).map(plugin => {
    log.warning(
      'Running deprecated function ' +
        fN +
        ' in plugin ' +
        plugin.name() +
        '. Please switch use messages see https://www.sitespeed.io/link/to/doc'
    );
    return Promise.resolve(plugin[fN].apply(plugin, args));
  });
}
      ...
      .then(pluginNames => {
        return loader.loadPlugins(pluginNames).then(plugins => {
          ...
          return runOptionalFunction(
            ...
            .then(() =>
              runDeprecatedOptionalFunction(allPlugins, 'postOpen', options)
            )
