...
Sitespeed.prototype.rerun = function(resultJson, outputDir, config, cb) {
  var result = require(resultJson);
  result.config.run.absResultDir = outputDir;
  // TODO merge config
  // createDataDir
  this._createOutput(cb);
};
...
Sitespeed.prototype._createOutput = function(callBack) {
  ...
  if (this.config.postTasksDir) {
    rootPath = path.join(this.config.postTasksDir, path.sep);
    log.log('info', 'Reading post tasks from ' + this.config.postTasksDir);
    fs.readdirSync(rootPath).forEach(function(file) {
      work.push(function(cb) {
        require(rootPath + file).task(result, self.config, cb);
      });
