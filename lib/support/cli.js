...
function loadPrePostTasks(tasks) {
  return util.toArray(tasks).map((task) => {
    try {
      ...
      console.error('Could not load task file ' + task);
...
module.exports.parseCommandLine = function parseCommandLine() {
  ...
  try {
    if (argv.browsertime && argv.browsertime.preTask) {
      argv.browsertime.preTask = loadPrePostTasks(argv.browsertime.preTask);
    }
    if (argv.browsertime && argv.browsertime.postTask) {
      argv.browsertime.postTask = loadPrePostTasks(argv.browsertime.postTask);
    }
  } catch (e) {
    process.exit(1);
  }
