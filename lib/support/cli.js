...
module.exports.parseCommandLine = function parseCommandLine() {
  let parsed = yargs
    ...
    .option('browsertime.script', {
      describe: 'Add custom Javascript to run after the page has finished loading. Note that --script can be passed multiple times if you want to collect multiple metrics. The metrics will automatically be pushed to the summary/detailed summary and each individual page + sent to Graphite/InfluxDB.',
      alias: ['script'],
      group: 'Browser'
    })
    ...
    .option('browsertime.delay', {
      describe: 'Delay between runs, in milliseconds. Use it to if you need your web server to rest between runs :)',
      group: 'Browser'
    })
  /*
   Crawler options
