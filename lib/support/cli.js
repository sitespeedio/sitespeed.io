...
let yargs = require('yargs'),
  ...
  influxdbConfig = require('../plugins/influxdb').config,
  elasticsearchConfig = require('../plugins/elasticsearch').config,
...
module.exports.parseCommandLine = function parseCommandLine() {
  ...
  .option('crawler.depth', {
    ...
    /**
     * Elasticsearch cli options
     */
        .option('elasticsearch.host', {
            describe: 'The Elasticsearch host used to store captured metrics.',
            group: 'Elasticsearch'
        })
        .option('elasticsearch.port', {
            default: elasticsearchConfig.port,
            describe: 'The Elasticsearch port used to store captured metrics.',
            group: 'Elasticsearch'
        })
        .option('elasticsearch.auth', {
            describe: 'The Elasticsearch user and password used for authentication. Format: user:password',
            group: 'Elasticsearch'
        })
        .option('elasticsearch.httpPort', {
            describe: 'The Elasticsearch port used to access the user interface and send annotations event',
            default: 8080,
            group: 'Elasticsearch'
        })
        .option('elasticsearch.webHost', {
            describe: 'The elasticsearch-web host. If not specified elasticsearch.host will be used.',
            group: 'Elasticsearch'
        })
        .option('elasticsearch.namespace', {
            default: elasticsearchConfig.namespace,
            describe: 'The namespace key added to all captured metrics.',
            group: 'Elasticsearch'
        })
        .option('elasticsearch.includeQueryParams', {
            default: elasticsearchConfig.includeQueryParams,
            describe: 'Whether to include query parameters from the URL in the Elasticsearch keys or not',
            type: 'boolean',
            group: 'Elasticsearch'
        })
        .option('elasticsearch.arrayTags', {
            default: false,
            type: 'boolean',
            describe: 'Send the tags as array or a string. In Elasticsearch 1.0 the tags is a array.',
            group: 'Elasticsearch'
        })
  /*
