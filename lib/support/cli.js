...
module.exports.parseCommandLine = function parseCommandLine() {
  ...
  .option('webpagetest.host', {
    ...
    .option('slack.channel', {
      describe: 'The slack channel to use',
      default: slackConfig.channel,
      group: 'Slack'
    })
    .option('slack.type', {
      describe: 'Send summary for a run, metrics from all URLs or both to Slack.',
      default: slackConfig.type,
      choices: ['summary', 'url', 'both'],
      group: 'Slack'
    })
  ...
  .option('mobile', {
    ...
    */
    ...
    .coerce('budget', function (arg) {
      return JSON.parse(require('fs').readFileSync(arg, 'utf8'))
    })
