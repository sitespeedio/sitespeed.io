...
module.exports = {
  parsePluginNames(rawConfig) {
    ...
    return fs.readdirAsync(defaultPluginsDir)
  ...
  loadPlugins(pluginNames, options) {
    return Promise.resolve(pluginNames).map((name) => {
      try {
        return require(path.join(defaultPluginsDir, name));
      } catch (err) {
        try {
          if (options.plugins.dir) {
            return require(path.resolve(path.join(options.plugins.dir, name)));
          }
        }
        catch (err) {
