...
class StorageManager {
  ...
  createDataDirForUrl(prePath, url, postPath) {
    return Promise.resolve(path.join(this.baseDir, prePath,  this.relativePathFromUrl(url), postPath))
      .tap((dirPath) => mkdirp(dirPath));
  }

  writeDataForUrl(url, prePath, filename, postPath, data) {
    return write(this.createDataDirForUrl(prePath, url, postPath), filename, data);
  }
}
