	...
	if ((this.config.testData.indexOf('page') > -1 || this.config.testData.indexOf(
		...
		Object.keys(this.budget.page).forEach(function(budget) {
			...
			if (budget === 'requests') {
				...
				var value = pageData.yslow.comps.length;

				results.push(self._getResult('Page requests',
					url,
					value < self.budget.page[budget],
					'The number of requests is ' + value + ' and the limit is ' + self.budget.page[budget],
					value,
					'page'));

			} else if (budget == 'pageWeight') {
				var value = yslowUtil.getSize(pageData.yslow.comps);

				results.push(self._getResult('Page requests',
					url,
					value < self.budget.page[budget],
					'The page size ' + value + ' (bytes) and the limit is ' + self.budget.page[budget],
					value,
					'page'));
			...
			assetTypes.forEach(function(asset) {
				...
				if (budget === asset) {
					...
					results.push(self._getResult('Page ' + asset + ' requests',
						url,
						value < self.budget.page[budget],
						'Page ' + asset + ' requests + ' + value + ' the limit is ' + self.budget.page[budget],
						value,
						'page'));

				} else if (budget === asset + 'Weight') {

					var value = yslowUtil.getSize(pageData.yslow.comps.filter(function(c) {
						return c.type === asset;
					}));

					results.push(self._getResult('Page ' + asset + ' weight',
						url,
						value < self.budget.page[budget],
						'Page ' + asset + ' weight ' + value + ' (bytes), the limit is ' + self.budget.page[budget],
						value,
						'page'));
...
TestRenderer.prototype._phantomJS = function(url, pageData) {
	var results = [];
	if ((this.config.testData.indexOf('timings') > -1 || this.config.testData.indexOf(
			'all') > -1) && pageData.phantomjs && this.budget.timings) {
		var self = this;
		Object.keys(this.budget.timings).forEach(function(timing) {
