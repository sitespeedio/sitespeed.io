...
TestRenderer.prototype._wpt = function(url, pageData) {
  ...
  if ((this.config.testData.indexOf('wpt') > -1 || this.config.testData.indexOf(
    ...
    var median = pageData.webpagetest.wpt.response.data.median.firstView;
        ...
        if (self.config.budget) {
          ...
          self.results.forEach(function(result) {
            ...
            if (result.isOk) {
              ...
              self.config.budgetIsFailing = true;
              self.log.log('info', 'The budget for ' + result.title + ' ' + result.url + ' ' + ' failed (' + result.value+ ') ' + result.description);
            }
          });
          callback();
