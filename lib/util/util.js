...
var crypto = require('crypto'),
  ...
  log = require('winston').loggers.get('sitespeed.io'),
...
module.exports = {
  ...
  getURLFromPageData: function(pageData) {
    if (pageData.yslow) {
      ...
      return pageData.webpagetest.wpt.response.data.testUrl;
    }
    return 'undefined';
  },
  ...
  getGraphiteURLKey: function(theUrl) {
    ...
    var key = protocol + '.' + hostname + path.split('/').join('.');
  ...
  logVersions: function(cb) {
    async.parallel([
        ...
        function(callback) {
          childProcess.execFile(phantomjsPath, ['--version'], {
            timeout: 120000
      ...
      function(err, results) {
        log.log('info', 'OS: ' + os.platform() + ' ' + os.release() + ' sitespeed:' + require('../../package.json').version +
          ' ' + ' phantomJs:' + results[0].replace(EOL, '') + ' java:' + results[1].split(EOL)[0] + ' browsertime:' + browserTimeVersion);
        cb(null, null);
      });
