...
module.exports = {
	...
	getGraphiteURLKey: function(theUrl, char) {
		...
		if (pathName === '' || pathName === '/') {
			return protocol + char + hostname + char + 'slash' + char;
		}

		var key = protocol + char + hostname + pathName.split('/').join(char);
	...
	logVersions: function(config, cb) {
		...
		async.parallel([
			...
			function(err, results) {
				...
				log.info('OS: \'%s\', Node.js: \'%s\', sitespeed.io: \'%s\', PhantomJS: \'%s\', java: \'%s\', browsertime: \'%s\'',
